...
class RegistroActivity : AppCompatActivity() {
    ...
    private fun valida_registro(){
        ...
        if(!correo.isNullOrBlank() && !contra1.isNullOrBlank() &&
            ...
            if (contra1 == contra2){
                if (contra1.length >= 6) registrarFirebase(correo, contra1)
                else {
                    Toast.makeText(this, "La contraseña debe contener al menos 6 caracteres.",
        ...
        auth.createUserWithEmailAndPassword(correo, contra)
            .addOnCompleteListener(this) { task ->
                    if (task.isSuccessful) {
                        ...
                        Toast.makeText(baseContext, "${user.email} is now logged in.",
                            Toast.LENGTH_SHORT).show()
                        //updateUI(user)
                    } else {
